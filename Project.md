...
We propose a memory architecture for AI agents that organizes skills within a two-tier hierarchical graph. The **strategic tier** ($d=1$) holds reasoning scaffolds — abstract frames selected once per episode under an options/semi-MDP formalism. The **tactical tier** (flat) holds directly executable skills formed from experience and retained via utility-modulated Ebbinghaus decay. Tactical skills are admitted by a TD error pre-filter followed by LLM judgment, stored immediately, and pruned by decay. Periodically, a **sleep consolidation** event clusters surviving tactical memories and uses LLM judgment to abstract them into strategic scaffolds — the sole mechanism by which $d=1$ nodes are created. The system is framed as a two-tier extended semi-MDP. Memory retention follows a biologically-grounded Ebbinghaus decay formula modulated by a **utility salience denominator**, with two variants explored: a global scalar Q-value (Phase 1a, simpler) and a confidence-weighted mean over per-task-type Q-values (Phase 1b, more principled).
...
> **Status:** Phase 1 architecture confirmed. Node schema finalized. Q-value representation open between global scalar (Phase 1a) and per-task-type weighted mean (Phase 1b) — both documented below. Tactical retrieval technique is one remaining open design decision.
...
**Key data structures:**
- `SkillNode` — one per skill at any depth. Strategic ($d=1$) and tactical ($d=\tau$) nodes share the same class but populate different fields. Defined in §6.3.
...
- All new tactical nodes enter at depth `tau` (flat tactical layer). No exceptions.
...
- Retention: Ebbinghaus decay modulated by utility salience. Two variants: global scalar Q (Phase 1a) and confidence-weighted per-task-type mean (Phase 1b).
...
- $Q^\Omega$ initialization for spawned scaffolds: shrinkage-weighted mean over absorbed cluster members' Q-values — not zero. See §3.7.
...
$$\mathcal{M}_t = \left(\mathcal{G}_t,\ \{Q_i\},\ \{Q^{\Omega}_j(t_k)\},\ \lambda,\ \epsilon\right)$$

| Component | Description |
|---|---|
| $\mathcal{G}_t$ | Unified skill graph: $d=1$ strategic nodes + flat tactical layer |
| $\{Q_i\}$ | Tactical Q-estimates. Phase 1a: global scalar $Q_i$. Phase 1b: per-task-type dict $Q_i(t_k)$ with shrinkage-weighted mean. |
| $\{Q^{\Omega}_j(t_k)\}$ | Strategic option-values, per task type. **Separate from** $\{Q_i\}$ — never merged. |
...
$$Q_i \approx \mathbb{E}\!\left[\Delta R \mid s_i\right], \qquad \Delta R = R_{\text{with skill}} - R_{\text{without skill}}$$

### 3.2 Phase 1a — Global Scalar Q

A single Q-value per node, updated per retrieval regardless of task type:

$$Q_i \leftarrow Q_i + \alpha \Bigl[r_t + \gamma \max_{s_j \in \mathcal{N}(s_i)} Q_j - Q_i\Bigr]$$

**Decay salience:** $Q_i$ directly. `decay_rate` $= \lambda / (Q_i + \epsilon)$.

**Rationale for Phase 1a:** simpler to implement, faster to debug, sufficient for initial benchmarking. Cross-task utility differentiation is not available, but decay is well-defined and task-agnostic.

**Known limitation:** a skill that is high-utility on one task type and useless on others will have an averaged $Q_i$ that neither fully retains nor fully prunes it. Phase 1b addresses this.

### 3.3 Phase 1b — Per-Task-Type Q with Confidence-Weighted Mean

Q-values stored per task type, as in the original spec:

$$Q_i(t_k) \leftarrow Q_i(t_k) + \alpha \Bigl[r_t + \gamma \max_{s_j \in \mathcal{N}(s_i)} Q_j(t_k) - Q_i(t_k)\Bigr]$$

**Decay salience:** confidence-weighted mean across task types:

$$\bar{Q}_{i,w} = \frac{\sum_k w_{ik} \cdot Q_i(t_k)}{\sum_k w_{ik}}, \qquad w_{ik} = \frac{n_{ik}}{n_{ik} + \lambda_{\text{shrink}}}$$

`decay_rate` $= \lambda / (\bar{Q}_{i,w} + \epsilon)$.
...
**Rationale for Phase 1b:** more principled decay — a task-specific high-utility skill decays correctly even when the agent switches task types, because $\bar{Q}_{i,w}$ reflects cross-task average, not last-episode task. Required for any future transferability scoring (Phase 2).

> **Implementation note:** Start with Phase 1a. Switch to Phase 1b after initial debugging. Both share the same `SkillNode` schema — Phase 1a uses a single-key dict `{global: Q_i}` or a bare float; Phase 1b uses the full per-task-type dict. Document which variant is active in the run configuration.

### 3.4 TD Error

$$\delta_t = r_t + \gamma \max_{s_j \in \mathcal{N}(s_i)} Q_j(t_k) - Q_i(t_k)$$

$\delta_t$ drives the Q-update and serves as the pre-filter signal for tactical formation (§4.1).

### 3.5 Initialization

**Tactical nodes:** $Q_i = 0$ (Phase 1a) or $Q_i(t_k) = 0\ \forall t_k$ (Phase 1b). No utility prior at creation — decay will remove misjudged nodes quickly.

**Strategic nodes (sleep consolidation — spawn case):**
...
Shrinkage-weighted mean over the absorbed cluster's Q-values. Nodes with more evidence on $t_k$ contribute more. **Do not initialize spawned scaffolds to zero** — this causes systematic under-selection of new scaffolds post-consolidation (FeUdal Networks dead-layer problem, Vezhnevets et al., 2017).

**Strategic nodes (sleep consolidation — absorb case):** the absorbed $d=\tau$ node carries its Q-values forward. Apply dampening:

$$Q^{\Omega}_{\omega}(t_k) \leftarrow \rho \cdot Q_{\tau}(t_k), \qquad \rho \in (0.5,\ 1.0)$$

$\rho$ acknowledges that Q-values calibrated in the tactical competitive context may overestimate strategic value. Starting value: $\rho = 0.7$, swept in ablation.

### 3.6 Tactical Action Selection

$$a_t^{\tau} = \arg\max_{s_i \in \text{top-}k}\ Q_i(t_k)$$

Candidates shortlisted by semantic similarity, then Q-ranked. Retrieval technique open (§11).
...
CREATE TABLE skill_graph_state (
    node_id             TEXT PRIMARY KEY,
    depth               INTEGER NOT NULL,       -- 1 (strategic) or tau (tactical)
    ...
    consolidated        INTEGER DEFAULT 0,      -- boolean 0/1; d=tau only
                                                -- True once this node has been processed
                                                -- by a sleep consolidation event
    Q                   TEXT,                   -- Phase 1a: JSON float; Phase 1b: JSON dict[str,float]
    n                   TEXT,                   -- Phase 1a: JSON int; Phase 1b: JSON dict[str,int]
...
class SkillNode:
    ...
    # --- Provenance ---
    task_type_dominant: str          # argmax_k n(t_k). Dynamic — updated as n grows.
    ...
    # --- Tactical Utility (d=tau ONLY) ---
    # Phase 1a: Q is a single float stored as dict {"global": float}
    # Phase 1b: Q is a per-task-type dict[str, float]
    # Both variants use the same field; active variant documented in run config.
    Q: dict = field(default_factory=dict)
    n: dict = field(default_factory=dict)   # Phase 1a: {"global": int}; Phase 1b: dict[str,int]

    # --- Strategic Option-Value (d=1 ONLY) ---
    Q_omega: dict[str, float] = field(default_factory=dict)   # Separate from Q. Never merge.
    n_omega: dict[str, int] = field(default_factory=dict)

    # --- Retention ---
    decay_rate: float = 0.0          # λ / (Q_salience + ε). Always 0.0 for d=1.
    ...
    # --- Sleep Consolidation Bookkeeping (d=tau ONLY) ---
    consolidated: bool = False       # True once processed by any sleep consolidation event.
                                     # Drives the unconsolidated-count sleep trigger (§6.6.1).
                                     # Meaningless on d=1 nodes.
    ...
    def recompute_decay_rate(self, lambda_base: float, epsilon: float,
                              phase: str = "1b") -> None:
        """
        Recompute cached decay rate. TACTICAL NODES ONLY.
        Phase 1a: decay_rate = lambda_base / (Q["global"] + epsilon)
        Phase 1b: decay_rate = lambda_base / (Q_bar_w + epsilon)
        d=1 nodes: unconditionally set decay_rate = 0.0.
        """
        if self.depth == 1:
            self.decay_rate = 0.0
            return
        if phase == "1a":
            q_salience = self.Q.get("global", 0.0)
        else:
            q_salience = self._weighted_mean_utility()
        self.decay_rate = lambda_base / (q_salience + epsilon)

    def _weighted_mean_utility(self, lambda_shrink: float = 10) -> float:
        """Phase 1b only. Shrinkage-weighted mean over per-task-type Q dict."""
        if not self.Q:
            return 0.0
...
| `task_type_dominant` | Dynamic, not static. Updated at episode end from `n`. |
| `decay_rate` | Tactical: `λ / (Q_salience + ε)`. Strategic: always `0.0`. |
| `Q` vs `Q_omega` | Mutually exclusive by depth. Assert `depth == 1 ⟹ Q empty` and vice versa. |
| `consolidated` | $d=\tau$ only. Drives sleep trigger counter. Do not repurpose. |
| `evidence_ids` | Reservoir-sampled at cap $R$. Implement `add_evidence(eid)` with reservoir sampling. |
| `total_accessed` | `@property`. Never store separately. |
...
Tactical decay governs global graph membership. Using task-local $Q_i(t_k)$ as the salience denominator would make retention path-dependent on the last episode's task type, which is architecturally incoherent for a unified (non-partitioned) graph. The correct denominator is a task-agnostic utility measure.

**Phase 1a** uses $Q_i$ (global scalar) directly. **Phase 1b** uses $\bar{Q}_{i,w}$ (confidence-weighted mean). Both are task-agnostic.
...
| Q\_salience | Phase 1a: $Q_i$. Phase 1b: $\bar{Q}_{i,w}$ |
...
|---|---|---|
| Q\_salience $= 0$ | $\lambda / \epsilon$ — maximum | New nodes with no confirmed utility are pruned quickly |
| Q\_salience $\to 1$ | $\approx \lambda$ | High-utility nodes decay at base rate |
| $d=1$ | $0$ | Permanent |
| Phase 1a, single retrieval | $\lambda / (Q_i + \epsilon)$ | Well-defined from first retrieval |
| Phase 1b, single task type | $\bar{Q}_{i,w} = Q_i(t_{k_0})$ (shrinkage cancels) | Cold-start well-defined |
...
```python
node.recompute_decay_rate(lambda_base=graph.lambda_base, epsilon=graph.epsilon, phase=config.phase)
```
...
$$\text{count}_{\text{unconsolidated}} = |\{\, n \in \mathcal{G}_\tau : \neg n.\texttt{consolidated} \,\}|$$
...
for each episode:
    ...
    for each step t:
        ...
        update_Q(a_t, t_k, delta_t, alpha)
        a_t.n[t_k] = a_t.n.get(t_k, 0) + 1
        a_t.last_accessed_step = current_step
        a_t.recompute_decay_rate(graph.lambda_base, graph.epsilon, config.phase)
        ...
        if delta_t > theta_delta:
            ...
            for (s, step_s) in active_skills:
                ...
                update_Q(s, t_k, -penalty, alpha=1.0)
                s.recompute_decay_rate(graph.lambda_base, graph.epsilon, config.phase)
    ...
    for exp in approved:
        new_node = create_skill_node(exp)    # LLM extraction + embedding; depth=tau
        parent = G.root                      # all tactical nodes hang from root in Phase 1
...
| $\lambda_{\text{shrink}}$ | Bayesian shrinkage pseudocount (Phase 1b) | $10$ | sweep |
| $R$ | Evidence reservoir size per node | $50$ | sweep |
| $N_{\text{sleep}}$ | Unconsolidated tactical count triggering sleep | — | sweep |
| $\theta_{\text{consolidate}}$ | Minimum Q\_salience for consolidation eligibility | — | sweep |
...
| Utility signal | LLM-assessed at retrieval | Q-learning TD updates (tactical); option-value full-episode return (strategic) |
| Decay salience | N/A | Task-agnostic: global scalar $Q_i$ (Phase 1a) or shrinkage-weighted mean $\bar{Q}_{i,w}$ (Phase 1b) |
| Strategic scaffolds | None | Permanent $d=1$ nodes; never decay; initialized with cluster-mean Q-values, not zero |
